...
# ---------------------------------------------------------------------------
# Pod memory budget – regression gate for the production deployment profile
# ---------------------------------------------------------------------------
class TestPodMemoryBudget:
    """Verify the running app stays within the production pod memory envelope.

    The pod is configured with a 256 MB chDB server cap.  Python, native
    libraries, and OS overhead add on top of that.  The test starts a live
    SOBS server subprocess under exactly those constraints, exercises the
    most memory-intensive paths (bootstrap, ingest, UI pages), samples its
    RSS with psutil, and asserts the peak stays within the declared ceiling.

    The ceiling is intentionally conservative relative to the raw pod cap:
    the chDB 256 MB limit bounds the embedded DB engine; the Python process
    plus native libs typically adds another ~250 MB.  The test budget is set
    to 600 MB total RSS so that gradual growth is caught before it OOMs the
    pod while still leaving headroom above the current baseline (~400 MB).

    Override ``SOBS_TEST_POD_MEM_BUDGET_MIB`` to adjust for environment
    differences, e.g.::

        SOBS_TEST_POD_MEM_BUDGET_MIB=700 pytest -k test_server_stays_within_pod_memory_budget
    """

    # Pod's chDB env profile – mirrors the production deployment.
    _POD_ENV = {
    ...
    # Default per-process RSS ceiling for the server subprocess (MiB).
    _DEFAULT_BUDGET_MIB = int(os.environ.get("SOBS_TEST_POD_MEM_BUDGET_MIB", "600"))
    ...
    def _start_server(self, data_dir: str) -> "subprocess.Popen[bytes]":
        env = os.environ.copy()
        env.update(self._POD_ENV)
        env["PORT"] = str(self._SERVER_PORT)
    ...
    def _wait_ready(self, proc: "subprocess.Popen[bytes]") -> None:
        ...
        while time.time() < deadline:
            ...
            try:
                with urllib.request.urlopen(f"http://127.0.0.1:{self._SERVER_PORT}/health", timeout=1) as resp:
                    if resp.status == 200:
                        return
            except Exception:
                pass
            time.sleep(0.25)
        proc.terminate()
        raise RuntimeError(f"Server did not become ready within {self._STARTUP_TIMEOUT}s")

    def _exercise_server(self) -> None:
        """Send a realistic mix of ingest and UI requests to the live server."""
        import urllib.request

        base = f"http://127.0.0.1:{self._SERVER_PORT}"
        ts_ns = str(int(time.time() * 1_000_000_000))

        # Ingest: logs, traces, errors, RUM, metrics
        payloads = [
            (
                f"{base}/v1/logs",
                json.dumps(
                    {
                        "resourceLogs": [
                            {
                                "resource": {
                                    "attributes": [{"key": "service.name", "value": {"stringValue": "pod-profile-svc"}}]
                                },
                                "scopeLogs": [
                                    {
                                        "logRecords": [
                                            {
                                                "timeUnixNano": ts_ns,
                                                "severityText": "INFO",
                                                "body": {"stringValue": "pod profile log"},
                                            }
                                        ]
                                    }
                                ],
            ...
            (
                ...
                json.dumps(
                    {
                        "service": "pod-profile-svc",
                        "type": "RuntimeError",
                        "message": "pod profile error",
                        "stack": "app.py:1",
                    }
                ).encode(),
            ),
            (
                ...
                json.dumps(
                    {
                        "session_id": "pod-profile-session",
                        "event_type": "pageview",
                        "url": "/",
                        "data": {},
                    }
        ...
        # UI pages: hit each main section
        for path in ("/", "/logs", "/traces", "/errors", "/rum", "/metrics", "/ai"):
            try:
                with urllib.request.urlopen(f"{base}{path}", timeout=5):
                    pass
            except Exception:
                pass

    def test_server_stays_within_pod_memory_budget(self):
        """Server RSS under the pod chDB profile must stay within the declared budget."""
        if not self._psutil_available():
            pytest.skip("psutil not installed – install it to enable pod memory profiling")

        import psutil

        data_dir = tempfile.mkdtemp(prefix="sobs-pod-profile-")
        proc = self._start_server(data_dir)
        try:
            self._wait_ready(proc)
            root = psutil.Process(proc.pid)

            # Prime CPU% baseline before sampling loop.
            ...
            # Exercise under load while sampling RSS.
            # Run the workload twice to catch any post-first-request growth.
            self._exercise_server()
            self._exercise_server()

            max_rss_bytes = 0
            sample_end = time.monotonic() + 5.0
            while time.monotonic() < sample_end:
                ...
                try:
                    ...
                    pass
                max_rss_bytes = max(max_rss_bytes, rss)
                time.sleep(0.1)

            max_rss_mib = max_rss_bytes / (1024 * 1024)
            budget_mib = self._DEFAULT_BUDGET_MIB

            assert max_rss_mib <= budget_mib, (
                f"Pod memory budget exceeded: peak RSS {max_rss_mib:.1f} MiB "
                f"> budget {budget_mib} MiB. "
                f"Review recent changes for memory regressions or raise "
                f"SOBS_TEST_POD_MEM_BUDGET_MIB if the baseline legitimately grew."
            )
        finally:
            proc.terminate()
            try:
