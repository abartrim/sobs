...
> **Status:** 76 → **42 `not implemented` stubs remaining**. `parity_check.py` = **GREEN 271 / RED 0 / MISSING_GOLDEN 0 / UNCOVERED 0 / EXCLUDED 0**. Branch `claude/jolly-wu-5fc6a3` / PR #304. Also done: **G8 mcp** initialize/ping/notifications (unauthenticated); v1 `Allow` non-determinism fixed via `normalize.py` (method-set sort).
