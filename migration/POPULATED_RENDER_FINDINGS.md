...
| R10 | `view_metrics` rendered `Sample` `5.0` / `point_count` as floats vs Python ints | `last_sample_count`/`point_count` are UInts (`argMax(SampleCount,…)`/`count()`) passed raw (float64 from chdb); Python renders the int. `last_value`/`last_anomaly_score` are genuine Float64 → kept raw | `cInt(m, …)` for the two count columns (`handlers_pages.go`) — found by the `metricsauto` view_metrics batch (same class as R1) |
